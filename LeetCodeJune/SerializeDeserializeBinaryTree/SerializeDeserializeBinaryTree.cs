...
namespace LeetCodeJune.SerializeDeserializeBinaryTree
{
    public static class SerializeDeserializeBinaryTree
    {
        ...
        public static string Serialize([CanBeNull] TreeNode root)
        {
            ...
            while (queue.Any())
            {
                ...
                var currentRight = currentRoot.right;

                result.Add(currentLeft?.val.ToString());
                if (currentLeft != null)
                {
                    queue.Enqueue(currentLeft);
                    lastNotNullIndex = result.Count - 1;
                }

                result.Add(currentRight?.val.ToString());
                if (currentRight != null)
                {
                    queue.Enqueue(currentRight);
                    lastNotNullIndex = result.Count - 1;
                }
            }
        ...
        public static TreeNode Deserialize([NotNull] string str)
        {
            ...
            while (totalQueue.Any())
            {
                ...
                if (currentLeft != null)
                {
                    ...
                    currentQueue.Enqueue(currentLeft);
                }

                if (currentRight != null)
                {
                    currentRoot.right = currentRight;
                    currentQueue.Enqueue(currentRight);
                }
            }
    ...
    public class SerializeDeserializeBinaryTreeTests
    {
        ...
        {
            AssertSerializeDeserialize(null, "[]");
        }
        ...
        {
            var root = new TreeNode(1, right: new TreeNode(3));
            var expected = "[1,null,3]";
            AssertSerializeDeserialize(root, expected);
        }
        ...
        [Test]
        public void TestLongOnlyRightTree()
        {
            var root = new TreeNode();
            GrowRightRecursively(root, 0, 10);
            var expected = "[0,null,1,null,2,null,3,null,4,null,5,null,6,null,7,null,8,null,9,null,10]";
            var actual = SerializeDeserializeBinaryTree.Serialize(root);
            actual.Should().BeEquivalentTo(expected);
        }

        [Timeout(5000)]
        [Test]
        public void TestPerformanceForGiantTree_OnlyRight()
        {
            var input =
            ...
            var deserialized = SerializeDeserializeBinaryTree.Deserialize(input);
            var serialized = SerializeDeserializeBinaryTree.Serialize(deserialized);
            serialized.Should().BeEquivalentTo(input);
        }


        [Test]
        public void TestFullLevel3Tree()
        {
        ...
        private void AssertSerializeDeserialize(TreeNode root, string expectedSerialized)
        {
            ...
            var deserialized = SerializeDeserializeBinaryTree.Deserialize(serialized);
            Console.WriteLine($"Deserialized tree:\r\n{root.Print()}");

            serialized.Should().Be(expectedSerialized);
            deserialized.Should().BeEquivalentTo(root);
        }
    }
}