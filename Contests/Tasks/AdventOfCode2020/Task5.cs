...
namespace Contests.Tasks.AdventOfCode2020
{
    public static class Task5
    {
        ...
        public static Seat ParseSeatPosition(string seatPositionRaw)
        {
            var row = ParseBinaryPartition(seatPositionRaw.Substring(0, 7), zero: 'F', one: 'B');
            var column = ParseBinaryPartition(seatPositionRaw.Substring(7, 3), zero: 'L', one: 'R');
            return new Seat(row: row, column: column);
        }

        private static int ParseBinaryPartition(string binaryPartition, char zero, char one)
        {
            return Convert.ToInt32(binaryPartition.Replace(zero, '0').Replace(one, '1'), 2);
        }
    }
